...
DROP VIEW IF EXISTS results_psra_bc.psra_bc5920a1_economic_security_economic_loss_s CASCADE;
CREATE VIEW results_psra_bc.psra_bc5920a1_economic_security_economic_loss_s AS 
...
RIGHT JOIN psra_bc.psra_bc5920a1_avg_losses_stats h ON a.id = h.asset_id
...
DROP VIEW IF EXISTS results_psra_bc.psra_bc5920a1_economic_security_pml_s CASCADE;
CREATE VIEW results_psra_bc.psra_bc5920a1_economic_security_pml_s AS
...
FROM psra_bc.psra_bc5920a1_agg_curves_stats a;



-- src loss values
DROP VIEW IF EXISTS results_psra_bc.psra_bc5920a1_src_loss CASCADE;
CREATE VIEW results_psra_bc.psra_bc5920a1_src_loss AS

SELECT 
source AS "src_zone",
trt AS "src_type",
SUM(loss_value_b0)/(SELECT SUM(loss_value_b0) FROM psra_bc.psra_bc5920a1_src_loss ) AS "src_value_b0",
SUM(loss_value_r2)/(SELECT SUM(loss_value_r2) FROM psra_bc.psra_bc5920a1_src_loss ) AS "src_value_r2"
FROM psra_bc.psra_bc5920a1_src_loss
GROUP BY source,trt;
...
a.trt AS "src_type",
SUM(a.loss_value)/(SELECT SUM(loss_value) FROM psra_bc.psra_bc5920a1_src_loss_b0 ) AS "src_value_b0",
SUM(b.loss_value)/(SELECT SUM(loss_value) FROM psra_bc.psra_bc5920a1_src_loss_b0 ) AS "src_value_r2"
FROM psra_bc.psra_bc5920a1_src_loss_b0 a
INNER JOIN psra_bc.psra_bc5920a1_src_loss_b0 b ON a.source = b.source AND a.trt = b.trt
GROUP BY a.source,a.trt;

SELECT
SUM(loss_value) AS "TLossValue"
FROM psra_bc.psra_bc5920a1_src_loss_b0
GROUP BY source;
...
SUM(loss_value)/(SELECT SUM(loss_value) FROM psra_bc.psra_bc5920a1_src_loss_b0 )*100 AS "total_srce_p"
FROM psra_bc.psra_bc5920a1_src_loss_b0
GROUP BY source
);
...
SUM(loss_value) AS "total_loss_source",
SUM(loss_value)/(SELECT SUM(loss_value) FROM psra_bc.psra_bc5920a1_src_loss_b0 )*100 AS "total_srce_p"
FROM psra_bc.psra_bc5920a1_src_loss_r2
GROUP BY source
);
...
SUM(loss_value)/(SELECT SUM(loss_value) FROM psra_bc.psra_bc5920a1_src_loss_b0 )*100 AS "total_lt_p"
FROM psra_bc.psra_bc5920a1_src_loss_b0
GROUP BY loss_type
);
...
SUM(loss_value)/(SELECT SUM(loss_value) FROM psra_bc.psra_bc5920a1_src_loss_b0 )*100 AS "total_srce_lt_p"
FROM psra_bc.psra_bc5920a1_src_loss_b0
...
SUM(loss_value)/(SELECT SUM(loss_value) FROM psra_bc.psra_bc5920a1_src_loss_b0 )*100 AS "total_srce_lt_trt_p"
FROM psra_bc.psra_bc5920a1_src_loss_b0
